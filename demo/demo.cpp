...
class RenderContext {
public:
    RenderContext(const std::array<float, 2>& origin, float scale, bool flipY = false):
        scale(scale), flipY(flipY) {
...
public:
    ...
    void draw(std::array<float, 2> offset) const {
            ...
            renderContext.drawCircleFilled(origin, drawSpecs.headSize, BLACK);
            ...
            renderContext.drawLine(origin, foot, drawSpecs.bodyWidth, BLACK);
        }
...
public:
    ...
    Terrain(int length, int top_width, int bottom_width, int thickness,
            int center_x, int difficulty_level) :
        trapezoid_length(length),
...
private:
    ...
    void initialize_platforms(int num_platforms, int platform_thickness) {
        ...
        for (int i = 0; i < num_platforms; ++i) {
            if (previous_y + 120 > trapezoid_length - 120) break;
        ...
        if (!platforms.empty()) {
            ...
            door_position = {
                ...
                first_platform.y - 30, 20, 30, &first_platform
            };
        }
    }
    ...
    int calculatePlatformX(int index, int current_width, int platform_width) {
        if (index % 3 == 0) {
            return random_range(0, 1) == 0 ?
                   screen_center_x - current_width / 2 + edge_thickness :
                   screen_center_x + current_width / 2 - edge_thickness - platform_width;
        } else {
    ...
    void updateAndDrawDoor(int y) {
        glColor3ub(0, 0, 255);  // Set color to blue for the door
...
class Bouncing {
public:
    Bouncing(Scoring& score): score(score){
    }
...
int main() {
    ...
    while(gameRunning) {
        ...
        Terrain terrain(1000, 800, 400, 10, 500, difficulty.get_difficulty());
        Scoring scoring;
        Bouncing bouncing(scoring);
